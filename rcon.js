...
export default class Rcon extends EventEmitter {
  constructor(options = {}) {
    ...
    onPacket(decodedPacket) {
      ...
      // console.log(`Processing decoded packet: ${this.decodedPacketToString(decodedPacket)}`)
      ...
      case SERVERDATA_AUTH_RESPONSE:
        switch (decodedPacket.id) {
          ...
          default:
            // console.log(`Unknown packet ID ${decodedPacket.id} in: ${this.decodedPacketToString(decodedPacket)}`)
      ...
      default:
        // console.log(`Unknown packet type ${decodedPacket.type} in: ${this.decodedPacketToString(decodedPacket)}`)

        this.onClose('Unknown Packet')
      }
  }
  decodeData(data) {
    // console.log(`Got data: ${this.bufToHexString(data)}`);
    ...
    while (this.incomingData.byteLength >= 4) {
      ...
      if (this.incomingData.byteLength < packetSize) {
        // console.log(`Waiting for more data... Have: ${this.incomingData.byteLength} Expected: ${packetSize}`);
        break;
      }
      const packet = this.incomingData.slice(0, packetSize);

      // console.log( `Processing packet: ${this.bufToHexString(packet)}`);
      ...
      if (size === 10 && this.incomingData.byteLength >= 21) {
        ...
        if (decodedProbePacket.body === '\x00\x00\x00\x01\x00\x00\x00') {
          ...
          // console.log(`Ignoring some data: ${this.bufToHexString(probeBuf)}`);
          continue;
        }
      }
  ...
  onClose(hadError) {
    this.connected = false;
    this.loggedin = false;
    // console.log(`Socket closed ${hadError ? 'with' : 'without'} an error. ${hadError}`);

    // Cleanup all local state onClose
    if (this.incomingData.length > 0) {
      // console.log(`Clearing Buffered Data`);
      this.incomingData = Buffer.from([]);
    }
    if (this.incomingResponse.length > 0) {
      // console.log( `Clearing Buffered Response Data`);
      this.incomingResponse = [];
    }
    if (this.responseCallbackQueue.length > 0) {
      // console.log( `Clearing Pending Callbacks`);
    ...
    if (this.autoReconnect) {
      // console.log(`Sleeping ${this.autoReconnectDelay}ms before reconnecting.`);
  ...
  connect() {
    return new Promise((resolve, reject) => {
      ...
      const onError = (err) => {
        ...
        // console.log(`Failed to connect to: ${this.host}:${this.port}`, err);
   ...
   disconnect() {
    return new Promise((resolve, reject) => {
      ...
      const onError = (err) => {
        ...
        // console.log(`Failed to disconnect from: ${this.host}:${this.port}`, err);
  ...
  write(type, body) {
    return new Promise((resolve, reject) => {
      ...
      // console.log( `Writing packet with type "${type}" and body "${body}".`);
      ...
      const onError = (err) => {
        // console.log('Error occurred. Wiping response action queue.', err);
        this.responseCallbackQueue = [];
        reject(err);
      };
      ...
      if (type === SERVERDATA_AUTH) {
        ...
        this.responseCallbackQueue.push((response) => {
          ...
          if (response instanceof Error) {
            ...
            // console.log(`Returning complete response: ${response.replace(/\r\n|\r|\n/g, '\\n')}`);
            resolve(response);
          }
        });
      }
      ...
      // console.log(`Sending packet: ${this.bufToHexString(encodedPacket)}`);
      this.client.write(encodedPacket);

      if (type !== SERVERDATA_AUTH) {
        // console.log(`Sending empty packet: ${this.bufToHexString(encodedEmptyPacket)}`);
